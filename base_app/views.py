...
def login(request):
    ...
    if request.method == 'POST':
        ...
        elif user_registration.objects.filter(email=request.POST['email'], password=request.POST['password'],designation_id=design2.id).exists():
    ...
    if request.method == 'POST':
        ...
        for ins_screenshot in lst_screenshot:
            ...
            if ins_screenshot:
                ...
                print(lst_file)
                test.json_testerscreenshot = lst_file
        test.save()
        base_url = reverse('TSprojectdetails', kwargs={'pid': test.project_id})
        query_string = urlencode({'pid': test.project_id})
        url = '{}?{}'.format(base_url, query_string)
